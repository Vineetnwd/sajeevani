...
$currentPage = 'purchase_returns.php'; // Keep sidebar active on Purchase Returns
...
<head>
    ...
    <title>Create Purchase Return - <?php echo APP_NAME; ?></title>
...
<body class="bg-gray-50 flex h-screen overflow-hidden">
    ...
    <main class="flex-1 flex flex-col h-full bg-gray-50 overflow-hidden">
        <header class="bg-white/80 backdrop-blur-md shadow-sm border-b border-gray-200 z-10 px-4 py-3 sm:px-6 sm:py-4 flex justify-between items-center sticky top-0">
            <div class="flex items-center gap-3 sm:gap-4 min-w-0">
                <a href="purchase_returns.php" class="text-gray-400 hover:text-gray-600 transition-colors">
                    <svg class="w-6 h-6" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M10 19l-7-7m0 0l7-7m-7 7h18"></path></svg>
                </a>
                <div class="min-w-0">
                    <h1 class="text-lg sm:text-xl truncate font-bold text-gray-800">Process Purchase Return</h1>
                </div>
            </div>
            ...
            <form id="returnForm" class="max-w-5xl mx-auto space-y-6">
                ...
                <div class="bg-white p-6 rounded-2xl shadow-sm border border-gray-200">
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                        <div>
                            <label class="block text-sm font-semibold text-gray-700 mb-1">Return To (Dealer / Vendor)</label>
                            <select name="dealer_name" id="dealerSelect" required class="w-full border border-gray-300 rounded-lg p-2.5 text-sm focus:ring-2 focus:ring-orange-500 outline-none">
                        ...
                        <div>
                            ...
                            <input type="text" name="reason" placeholder="e.g. Expired, Damaged, Excess stock..." class="w-full border border-gray-300 rounded-lg p-2.5 text-sm focus:ring-2 focus:ring-orange-500 outline-none">
                        </div>
                    </div>
                </div>
                        ...
                        <table class="w-full text-left" id="itemsTable">
                            <thead>
                                <tr class="text-xs text-gray-500 uppercase tracking-wider border-b border-gray-200">
                                    ...
                                    <th class="pb-3 font-semibold w-40">Return Rate (₹)</th>
                ...
                <div class="flex justify-end">
                    <div class="bg-orange-50 border border-orange-100 p-6 rounded-2xl w-full sm:w-80 text-right shadow-sm">
                        <p class="text-orange-600 font-bold uppercase text-xs tracking-wider mb-1">Return Value</p>
    ...
    <div id="successModal" class="fixed inset-0 bg-gray-900 bg-opacity-50 hidden flex justify-center items-center z-50">
        <div class="bg-white rounded-2xl p-8 max-w-sm w-full text-center shadow-2xl transform transition-all">
            ...
            <p class="text-gray-600 mb-6 font-medium">Purchase Return recorded successfully. Stock has been deducted.</p>
            <a href="purchase_returns.php" class="block w-full bg-orange-500 hover:bg-orange-600 text-white font-bold py-3 px-4 rounded-xl transition-colors">View Returns</a>
        </div>
    </div>
    ...
    <script>
        ...
        document.getElementById('returnForm').addEventListener('submit', async function(e) {
            ...
            try {
                ...
                const response = await fetch('api/purchase_return_api.php', {
                    method: 'POST',
                    body: formData
                });
