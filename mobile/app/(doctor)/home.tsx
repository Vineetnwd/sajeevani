...
import { StyleSheet, View, Text, ScrollView, TouchableOpacity, ActivityIndicator, RefreshControl, StatusBar, Alert, Modal, TextInput } from 'react-native';
            ...
            return (
              <View key={order.id} style={styles.orderCard}>
                <View style={styles.orderCardTop}>
                  ...
                  <View style={{ alignItems: 'flex-end' }}>
                    ...
                    {order.payment_status === 'Paid' && (
                      <View style={[styles.statusBadge, { backgroundColor: '#D1FAE5', marginTop: 4, paddingVertical: 2 }]}>
                        <Ionicons name="checkmark-circle" size={10} color="#059669" style={{marginRight: 4}} />
                        <Text style={[styles.statusText, { color: '#047857', fontSize: 9 }]}>PAID</Text>
                      </View>
                    )}
                  </View>
                </View>
                ...
                {order.status_remarks && (
                  <View style={[styles.notesRow, { backgroundColor: '#F0FDF4', borderLeftColor: '#059669' }]}>
                    <Text style={styles.notesText}><Text style={{fontWeight:'700', color:'#064E3B'}}>Remark: </Text>{order.status_remarks}</Text>
                  </View>
                )}
                {order.status === 'Dispatched' && (order.courier_company || order.awb_no) && (
                  <View style={[styles.notesRow, { backgroundColor: '#F0FDF4', borderLeftColor: '#059669', flexDirection: 'column', alignItems: 'flex-start' }]}>
                     {order.courier_company && <Text style={styles.notesText}><Text style={{fontWeight:'700', color:'#064E3B'}}>Courier: </Text>{order.courier_company}</Text>}
                     {order.awb_no && <Text style={[styles.notesText, { marginTop: 2 }]}><Text style={{fontWeight:'700', color:'#064E3B'}}>AWB: </Text>{order.awb_no}</Text>}
                  </View>
                )}
                ...
                {order.payment_status === 'Paid' && (
                  <TouchableOpacity 
                    style={[styles.markDeliveredBtn, { backgroundColor: '#F8FAFC', borderWidth: 1, borderColor: '#E2E8F0', elevation: 0, marginTop: order.status === 'Dispatched' ? 8 : 12 }]} 
                    onPress={() => router.push({ pathname: '/(doctor)/receipt', params: { orderId: order.id } })}
                  >
                    <Ionicons name="receipt-outline" size={18} color="#0F172A" />
                    <Text style={[styles.markDeliveredBtnText, { color: '#0F172A' }]}>View Payment Receipt</Text>
                  </TouchableOpacity>
                )}
              </View>
            );
...
const styles = StyleSheet.create({
  ...
  headerWrapper: {
    ...
    elevation: 0,
    zIndex: 10,
    marginTop: -50, // To cover the notch area behind SafeArea
    paddingTop: 50 + 16, // Add back the padding
  },
  ...
  statNum: { fontSize: 22, fontWeight: '800', color: '#064E3B', marginVertical: 4 },
  ...
  orderAmount: { fontSize: 15, fontWeight: '800', color: '#059669', textAlign: 'right' },
  ...
  modalContent: { backgroundColor: '#fff', width: '85%', borderRadius: 16, padding: 24, shadowColor: '#000', shadowOpacity: 0.1, shadowRadius: 10, elevation: 10 },
  modalTitle: { fontSize: 18, fontWeight: '800', color: '#064E3B', marginBottom: 6 },
  modalSubtitle: { fontSize: 13, color: '#475569', marginBottom: 20 },
  modalLabel: { fontSize: 13, fontWeight: '700', color: '#0F172A', marginBottom: 8 },
  modalInput: { backgroundColor: '#F8FAFC', borderRadius: 10, padding: 12, fontSize: 14, color: '#1E293B', borderWidth: 1, borderColor: '#E2E8F0', minHeight: 80, marginBottom: 20 },
  ...
  modalConfirmBtn: { backgroundColor: '#10B981', paddingHorizontal: 20, paddingVertical: 12, borderRadius: 10, minWidth: 100, alignItems: 'center' },
