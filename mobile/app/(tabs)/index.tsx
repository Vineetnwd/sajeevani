...
import { StyleSheet, View, Text, FlatList, TouchableOpacity, ActivityIndicator, Alert, TextInput, Modal, StatusBar, RefreshControl, useColorScheme } from 'react-native';
      ...
      <Modal visible={editModalVisible} animationType="slide" transparent={true}>
        <View style={styles.modalOverlay}>
          <View style={styles.modalContent}>
            ...
            <TextInput style={styles.input} placeholderTextColor={isDark ? '#9CA3AF' : '#9CA3AF'} value={editFormData.name} onChangeText={t => setEditFormData(prev => ({ ...prev, name: t }))} />

            <Text style={styles.label}>Phone Number</Text>
            <TextInput style={styles.input} placeholderTextColor={isDark ? '#9CA3AF' : '#9CA3AF'} value={editFormData.phone} keyboardType="phone-pad" onChangeText={t => setEditFormData(prev => ({ ...prev, phone: t }))} />

            <Text style={styles.label}>Symptoms</Text>
            <TextInput style={[styles.input, { height: 100, textAlignVertical: 'top' }]} placeholderTextColor={isDark ? '#9CA3AF' : '#9CA3AF'} multiline value={editFormData.symptoms} onChangeText={t => setEditFormData(prev => ({ ...prev, symptoms: t }))} />
...
const getStyles = (isDark: boolean) => StyleSheet.create({
  ...
  avatarContainer: { width: 36, height: 36, borderRadius: 18, backgroundColor: isDark ? '#065F46' : '#D1FAE5', justifyContent: 'center', alignItems: 'center' },
  ...
  statNumber: { fontSize: 22, fontWeight: '800', color: isDark ? '#34D399' : '#064E3B', marginVertical: 4 },
  ...
  // Card Styles
  card: { backgroundColor: isDark ? '#1F2937' : '#FFFFFF', borderRadius: 16, padding: 16, marginHorizontal: 16, marginBottom: 12, elevation: 2, shadowColor: '#000', shadowOpacity: isDark ? 0.3 : 0.05, shadowRadius: 5, shadowOffset: { width: 0, height: 2 } },
  ...
  avatarPlaceholder: { width: 40, height: 40, borderRadius: 20, backgroundColor: isDark ? '#065F46' : '#D1FAE5', justifyContent: 'center', alignItems: 'center', marginRight: 12 },
  avatarText: { fontSize: 16, fontWeight: '800', color: isDark ? '#34D399' : '#059669' },
  patientName: { fontSize: 15, fontWeight: '700', color: isDark ? '#F9FAFB' : '#0F172A', marginBottom: 2 },
  detailsText: { fontSize: 12, color: isDark ? '#9CA3AF' : '#64748B', fontWeight: '500' },
  badge: { paddingHorizontal: 10, paddingVertical: 4, borderRadius: 8, marginLeft: 8 },
  badgePending: { backgroundColor: isDark ? '#78350F' : '#FEF3C7' },
  badgeDone: { backgroundColor: isDark ? '#065F46' : '#D1FAE5' },
  badgeText: { fontSize: 11, fontWeight: '700' },
  badgeTextPending: { color: isDark ? '#FDE68A' : '#B45309' },
  badgeTextDone: { color: isDark ? '#6EE7B7' : '#047857' },

  divider: { height: 1, backgroundColor: isDark ? '#374151' : '#F1F5F9' },

  cardBody: { paddingTop: 12 },
  infoRow: { flexDirection: 'row', alignItems: 'center', marginBottom: 8 },
  symptomsText: { fontSize: 13, color: isDark ? '#D1D5DB' : '#334155', marginLeft: 8, flex: 1 },
  dateText: { fontSize: 12, color: isDark ? '#9CA3AF' : '#64748B', marginLeft: 8, fontWeight: '500' },

  editBtn: { flexDirection: 'row', backgroundColor: isDark ? '#065F46' : '#F0FDF4', padding: 12, borderRadius: 12, alignItems: 'center', justifyContent: 'center', marginTop: 10, borderWidth: 1, borderColor: isDark ? '#059669' : '#D1FAE5' },
  editBtnText: { color: isDark ? '#6EE7B7' : '#059669', fontWeight: '700', fontSize: 13 },

  emptyContainer: { alignItems: 'center', justifyContent: 'center', paddingVertical: 60 },
  emptyText: { marginTop: 12, color: isDark ? '#D1D5DB' : '#475569', fontSize: 16, fontWeight: '700' },
  emptySubtext: { marginTop: 4, color: isDark ? '#6B7280' : '#94A3B8', fontSize: 12 },

  // Modal Styles
  modalOverlay: { flex: 1, backgroundColor: 'rgba(17, 24, 39, 0.7)', justifyContent: 'flex-end' },
  modalContent: { backgroundColor: isDark ? '#1F2937' : '#fff', padding: 24, borderTopLeftRadius: 24, borderTopRightRadius: 24, shadowColor: '#000', shadowOffset: { width: 0, height: -4 }, shadowOpacity: isDark ? 0.3 : 0.1, shadowRadius: 12, elevation: 10 },
  modalHeader: { flexDirection: 'row', justifyContent: 'space-between', alignItems: 'center', marginBottom: 20 },
  modalTitle: { fontSize: 22, fontWeight: 'bold', color: isDark ? '#F9FAFB' : '#111827' },
  label: { fontSize: 14, color: isDark ? '#D1D5DB' : '#4B5563', marginBottom: 6, fontWeight: '600' },
  input: { borderWidth: 1, borderColor: isDark ? '#4B5563' : '#E5E7EB', borderRadius: 12, padding: 14, marginBottom: 16, backgroundColor: isDark ? '#374151' : '#F9FAFB', fontSize: 16, color: isDark ? '#F9FAFB' : '#1F2937' },
  modalActions: { flexDirection: 'row', justifyContent: 'flex-end', marginTop: 12 },
  cancelBtn: { padding: 14, marginRight: 8, borderRadius: 12, justifyContent: 'center' },
  cancelBtnText: { color: isDark ? '#9CA3AF' : '#6B7280', fontWeight: 'bold', fontSize: 16 },
  saveBtn: { flexDirection: 'row', backgroundColor: isDark ? '#10B981' : '#059669', paddingVertical: 14, paddingHorizontal: 20, borderRadius: 12, alignItems: 'center', elevation: 2, shadowColor: isDark ? '#10B981' : '#059669', shadowOpacity: 0.3, shadowRadius: 4, shadowOffset: { width: 0, height: 2 } },
